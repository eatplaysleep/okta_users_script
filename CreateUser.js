...
const RequestExecutorWithRateLimiting = require('./RequestExecutor');

function CreateUser() {
	this._client = new okta.Client({
		...oktaConfig.okta,
		requestExecutor: new RequestExecutorWithRateLimiting(),
	});
	this._count = 0;
...
function worker(user, callback) {
	...
	that._client
		...
		// })
		.then(oktaUser => callback())
		.catch(err => {
			console.log(err);
		});
}

CreateUser.prototype.update = function (user) {
	this._queue.push(user);
};

CreateUser.prototype.complete = function () {
	this._done = true;
};
